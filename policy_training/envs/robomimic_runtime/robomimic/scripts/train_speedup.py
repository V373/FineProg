...
import time
import os
import shutil  # noqa: F401  # 仍保留：latest/backup 保存逻辑被注释，但 resume 路径仍可能用到
import psutil
import sys
import socket
import traceback
...
def train(config, device, resume=False):
    ...
    # Input shapes are fixed across epochs (same obs keys, same batch size,
    # same dataset), so let cuDNN benchmark and pick the fastest convolution
    # algorithm.  This typically shaves a few percent off forward+backward
    # on image-based obs.
    ...
    #
    # Enabled by default on CUDA, controlled via config.train.amp_enabled.
    # Set amp_enabled: false in the config to disable if a specific algo or
    # dataset is numerically sensitive.
    # ------------------------------------------------------------------
    _amp_enabled = (
        device.type == "cuda"
        and getattr(config.train, "amp_enabled", True)
    )
    if _amp_enabled:
        _amp_scaler = torch.cuda.amp.GradScaler()
        TorchUtils.set_amp_scaler(_amp_scaler)
        print("AMP (float16 autocast + GradScaler) ENABLED")
    else:
        _amp_scaler = None
        print("AMP disabled (CPU or amp_enabled=false)")

    print("\n============= New Training Run with Config =============")
    print(config)
    print("")
    # Resolve task name for directory layout: prefer explicit override, else peek at dataset metadata
    _task_name_for_dir = config.experiment.env
    if _task_name_for_dir is None and config.train.data is not None:
        _first_path = config.train.data if isinstance(config.train.data, str) else config.train.data[0]["path"]
        try:
            _task_name_for_dir = FileUtils.get_env_metadata_from_dataset(
                dataset_path=os.path.expanduser(_first_path)
            )["env_name"]
        except Exception:
            _task_name_for_dir = None
    log_dir, ckpt_dir, video_dir, time_dir = TrainUtils.get_exp_dir(config, resume=resume, task_name=_task_name_for_dir)
    ...
    #
    # Speed knobs (only effective when num_data_workers > 0):
    #   pin_memory        – page-lock the host tensor so the H2D copy can be
    #                       issued as a DMA transfer and overlapped with GPU
    #                       compute.  Without this, the H2D is a synchronous
    #                       copy and the data-loading stage becomes serial
    #                       with the GPU stage even when workers exist.
    #   persistent_workers– keep the worker pool alive across epochs (we run
    #                       for 2000 epochs); otherwise each epoch pays the
    #                       full worker-spawn + HDF5-reopen cost.
    #   prefetch_factor   – queue more batches per worker so the next batch is
    #                       almost always ready when the GPU finishes the
    #                       current one.
    _nw = config.train.num_data_workers
    _use_workers = _nw > 0
    _pin_memory = getattr(config.train, "pin_memory", True) and _use_workers
    train_loader = DataLoader(
        ...
        persistent_workers=_use_workers and getattr(config.train, "persistent_workers", True),
        prefetch_factor=(4 if _use_workers else None),
        drop_last=True
    )
    ...
    if config.experiment.validate:
        ...
        _use_valid_workers = num_workers > 0
        ...
        valid_loader = DataLoader(
            ...
            pin_memory=_use_valid_workers and getattr(config.train, "pin_memory", True),
            persistent_workers=False,  # validation set is small; no need to keep workers alive
            drop_last=True
        )
    else:
        valid_loader = None
    ...
    _task_name = env_meta_list[0]["env_name"] if env_meta_list else None
    ...
    for epoch in range(start_epoch, config.train.num_epochs + 1):
        ...
        if config.experiment.rollout.enabled and (epoch > config.experiment.rollout.warmstart) and rollout_check:
            ...
            # NOTE: env_key 仍用于 envs 字典内部索引，但不再写进 wandb 路径 —— 单 env 训练时
            # 多余的 env_name 段只会让 panel 标题变长。这里把 {env_name}/{env_key} 从路径里去掉。
            # NOTE: 上传策略
            #   - Timing_Stats/Rollout_Episode（来自 Time_Episode 键）保留上传。
            #   - Rollout/* 这组基础指标（time / Return / Horizon / Success_Rate 等）保留上传。
            #   - 全部用 log_stats=False，避免 data_logger 额外写
            #     "Rollout/{key}/{mean,std,min,max}" 这组聚合 series（参见 log_utils.py
            #     DataLogger.record + get_stats）。这样 wandb 上只看得到 rollout/xx，
            #     不会再出现 rollout/success_rate/min max std mean 这种面板。
            for env_name in all_rollout_logs:
                rollout_logs = all_rollout_logs[env_name]
                for k, v in rollout_logs.items():
                    if k.startswith("Time_"):
                        # 原: "Timing_Stats/Rollout_{env_name}_{key}" -> 去掉 env_name
                        data_logger.record("Timing_Stats/Rollout_{}".format(k[5:]), v, epoch, log_stats=False)
                    else:
                        # 原: "Rollout/{key}/{env_name}" -> 去掉 env_name
                        data_logger.record("Rollout/{}".format(k), v, epoch, log_stats=False)

                print("\nEpoch {} Rollouts took {}s (avg) with results:".format(epoch, rollout_logs["time"]))
                print('Env: {}'.format(env_name))
                print(json.dumps(rollout_logs, sort_keys=True, indent=4))

            # upload rollout videos to wandb
            if config.experiment.render_video and video_paths:
                for env_key, vpath in video_paths.items():
                    # 原: "Rollout/video/{env_key}" -> 去掉 env_key，单 env 训练下不会冲突
                    data_logger.record_video("Rollout/video", vpath, epoch)
