...
class BaseConfig(Config):
    ...
    def train_config(self):
        ...
        # When num_data_workers > 0, page-lock the host tensor so the H2D
        # copy can be issued as a DMA transfer and overlapped with GPU
        # compute.  Has no effect when num_data_workers == 0.
        self.train.pin_memory = True

        # When num_data_workers > 0, keep the worker pool alive across
        # epochs.  Avoids paying the full worker-spawn + HDF5-reopen cost
        # for every epoch in a long run (e.g. 2000 epochs).
        self.train.persistent_workers = True

        # When num_data_workers > 0, queue this many batches per worker so
        # the next batch is almost always ready when the GPU finishes the
        # current one.  PyTorch default is 2; 4 is a better fit for
        # 2-view image obs (HDF5 + decode work is non-trivial).
        self.train.prefetch_factor = 4
        ...
        # Mixed-precision training toggle (used by train_speedup.py / TrainUtils.run_epoch)
        self.train.amp_enabled = False  # set True to wrap train_on_batch with torch.autocast
