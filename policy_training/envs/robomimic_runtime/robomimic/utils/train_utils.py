...
def get_exp_dir(config, auto_remove_exp_dir=False, resume=False, task_name=None):
    """
    ...
    Args:
        ...
        task_name (str): optional task/environment name inserted as an independent
            top-level subdirectory between experiment name and timestamp.
    ...
    Directory layout:
        <output_dir>/<experiment.name>/[<task_name>/]<reward_type>/<YYYYMMDD-HHMMSS>/{models,logs,videos}
    where <reward_type> is one of {original, dense, PBRS}, detected from the
    first training dataset's filename (see _detect_reward_type in log_utils).
    """
    # timestamp for directory names — format matches wandb run name (experiment_name-YYYYMMDD-HHMMSS)
    t_now = time.time()
    time_str = datetime.datetime.fromtimestamp(t_now).strftime('%Y%m%d-%H%M%S')

    # detect reward type from the training dataset path (mirrors the wandb run name tag)
    reward_type = LogUtils._detect_reward_type(config)
    ...
    if task_name:
        base_output_dir = os.path.join(base_output_dir, task_name)
    base_output_dir = os.path.join(base_output_dir, reward_type)
    if resume:
    ...
    # Each run gets its own timestamped subdirectory — no overwrite prompt needed.
        ...
        for step_i in range(horizon):
            ...
            if video_writer is not None:
                if video_count % video_skip == 0:
                    frame = env.render(mode="rgb_array", height=512, width=512) # 260602：这里修改了render分辨率，512x512 render存在bug
...
def run_epoch(model, data_loader, epoch, validate=False, num_steps=None, obs_normalization_stats=None, amp_enabled=False):
    """
    ...
    Args:
        ...
        amp_enabled (bool): if True, wrap train_on_batch with torch.autocast for mixed-precision
            training.  The GradScaler must have been registered via
            TorchUtils.set_amp_scaler() before calling this function.
    ...
    for _ in LogUtils.custom_tqdm(range(num_steps)):
        ...
        # When AMP is enabled, the forward pass (and loss computation inside train_on_batch)
        # runs in float16/bfloat16 via autocast.  The GradScaler (registered via
        # TorchUtils.set_amp_scaler) handles scaled backward + unscaled optimizer steps
        # inside backprop_for_loss.  scaler.update() is called once here, after ALL
        # optimizer steps for this batch are done (important for multi-optimizer algos).
        t = time.time()
        if amp_enabled and not validate:
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                info = model.train_on_batch(input_batch, epoch, validate=validate)
            scaler = TorchUtils.get_amp_scaler()
