...
# ---------------------------------------------------------------------------
# AMP (Automatic Mixed Precision) module-level scaler.
#
# Usage:
#   In the training entry-point, after creating a GradScaler, call
#       TorchUtils.set_amp_scaler(scaler)
#   to register it here.  backprop_for_loss() will then automatically use it
#   for all backward + optimizer-step calls.  The scaler.update() must be
#   called ONCE per training batch (after all optimizer steps), which
#   run_epoch() in train_utils.py handles.
# ---------------------------------------------------------------------------
_amp_scaler: "torch.cuda.amp.GradScaler | None" = None


def set_amp_scaler(scaler):
    """Register a GradScaler so that backprop_for_loss uses AMP automatically."""
    global _amp_scaler
    _amp_scaler = scaler


def get_amp_scaler():
    """Return the currently registered GradScaler, or None."""
    return _amp_scaler
...
def backprop_for_loss(net, optim, loss, max_grad_norm=None, retain_graph=False):
    ...
    if scaler is not None:
        ...
        # unscale before clipping so that the norm is in FP32 units
        if max_grad_norm is not None:
            scaler.unscale_(optim)
            torch.nn.utils.clip_grad_norm_(net.parameters(), max_grad_norm)
        scaler.step(optim)
        # NOTE: scaler.update() is intentionally NOT called here.
        # It must be called exactly once per training batch (after all optimizer
        # steps), which run_epoch() in train_utils.py handles.
    else:
    ...
    # compute grad norms (gradients are in FP32 units after unscale or plain backward)
    grad_norms = 0.
