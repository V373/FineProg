...
def _detect_reward_type(config):
    """
    Inspect the first training dataset's path and return a short tag describing
    which reward variant is encoded in the HDF5 filename. The dataset files
    created by the offline-RL pipeline follow the pattern
        low_dim_v15_reward_labeled[_original|_dense|_PBRS].hdf5
    so we just match substrings in the basename.

    Returns one of {"PBRS", "dense", "original"}. Falls back to "original"
    when no dataset is configured or the filename doesn't carry a known
    marker (e.g. the bare `low_dim_v15_reward_labeled.hdf5` is treated as
    original reward).
    """
    try:
    ...
    if isinstance(data, str):
        ...
        try:
            first_path = data[0]["path"]
        except (KeyError, IndexError, TypeError):
            return "original"
    name = os.path.basename(first_path).lower()
    if "pbrs" in name:
        return "PBRS"
    if "_dense" in name:
        return "dense"
...
class DataLogger(object):
    ...
    """
    def __init__(self, log_dir, config, log_tb=True, log_wandb=False, task_name=None):
        """
        Args:
            log_dir (str): base path to store logs
            log_tb (bool): whether to use tensorboard logging
            task_name (str): optional task/environment name included in the wandb run name
        """
        ...
        if log_wandb:
            ...
            # Insert a reward-type tag (original / dense / PBRS) into the run name
            # so different reward labelings are easy to compare in the wandb UI.
            _reward_tag = _detect_reward_type(config)
            if task_name:
                _run_name = "{}-{}-{}-{}".format(config.experiment.name, task_name, _reward_tag, _timestamp)
            else:
                _run_name = "{}-{}-{}".format(config.experiment.name, _reward_tag, _timestamp)
    ...
    def record_video(self, k, video_path, epoch):
        """
        Upload a video file to wandb.
        Args:
            k (str): key string (used as the wandb log key)
            video_path (str): path to the mp4 video file
            epoch: current epoch number
        """
        if self._wandb_logger is not None and os.path.isfile(video_path):
            try:
                import wandb
                self._wandb_logger.log({k: wandb.Video(video_path, fps=20, format="mp4")}, step=epoch)
            except Exception as e:
