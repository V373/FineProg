...
def to_device(x, device, non_blocking=False):
    ...
    Args:
        ...
        non_blocking (bool): if True, the host->device copy is issued
            asynchronously (cudaMemcpyAsync) and the call returns before the
            copy completes. Requires the source tensors to live in pinned
            (page-locked) host memory to actually overlap with GPU compute.
            Falls back to a synchronous copy when the source is not pinned,
            so it is always safe to pass True.
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            torch.Tensor: lambda x, d=device, nb=non_blocking: x.to(d, non_blocking=nb),
            type(None): lambda x: x,
        }
    )
