...
class IQL(PolicyAlgo, ValueAlgo):
    ...
    def process_batch_for_training(self, batch):
        ...
        # non_blocking=True lets the H2D copy overlap with the GPU's
        # previous-step work, as long as the source tensors live in pinned
        # host memory (which requires pin_memory=True on the DataLoader).
        # Falls back to a sync copy when the source is not pinned, so it
        # is always safe.
        return TensorUtils.to_device(TensorUtils.to_float(input_batch), self.device, non_blocking=True)
    ...
    def _compute_actor_loss(self, batch, critic_info):
        ...
        # Compute advantage-weighted actor loss.
        # weights = exp(adv / beta) come from _get_adv_weights; this is the AWR
        # (Advantage-Weighted Regression) objective. .detach() cuts the gradient
        # through the weights so the advantage estimator does not backprop into
        # the actor. Final loss = - E[ log_prob * w(s,a) ]; samples with lower
        # advantage are down-weighted and those with higher advantage are up-weighted.
        actor_loss_weighted = (-log_prob * weights.detach()).mean()
        # Training objective: plain MLE / behavior cloning loss without
        # advantage reweighting.
        actor_loss_unweighted = (-log_prob).mean()

        # Keep actor/loss as the actual training loss for backward compatibility.
        info["actor/loss"] = actor_loss_unweighted
        info["actor/loss_unweighted"] = actor_loss_unweighted
        info["actor/loss_weighted"] = actor_loss_weighted
        ...
        # Return stats
        return actor_loss_weighted, info
    ...
    def log_info(self, info):
        ...
        log["actor/loss_unweighted"] = info["actor/loss_unweighted"].item()
        log["actor/loss_weighted"] = info["actor/loss_weighted"].item()
